...
import {
  GetPain001TransactionNoteList,
  GetPain001TransactionNoteListPagination,
} from "../../Constants/AxiosHandler/request";
import DownloadImage from "../../../Assets/image/download (1).png";
import "./Pain001AccompanyingNote.css";
...
function Pain001AccompanyingNote() {
...
*/
  ...
  const SearchPain008TransactionNoteList = async (startindex =0,countOfrecords = 5) => {
    ...
    GetPain001TransactionNoteListCount(payload)
    const response = await GetPain001TransactionNoteList(payload);

    console.log(response);
    if (response.message == "") {
      setShowloaderMainTable(false);
      setShowMainTable(true)
      setPain008TransactionNoteList(response);
  ...
  const GetPain001TransactionNoteListCount = async (payload) => {
    ...
    const response = await GetPain001TransactionNoteListPagination(payload1);
    if(response.status=='success' && response.message==''){
      setTotalNoOfRecords(response.sepaFileListCount)
    }
  };
             ...
             ? (
              pain008TransactionNoteList.map((record, i) => {
                return (
                  <tr key={i}>
                    <td>{i+countSrNo}</td>
                    <td>{record.sepaFileId}</td>
                    <td>{Moment(record.dateMail).format('yyyy-MM-DD')}</td>
...
export default Pain001AccompanyingNote;
